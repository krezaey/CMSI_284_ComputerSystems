			...
			section	.text
add:
			mov		rax, rdi		; moving the first argument into the rax
			add		rax, rsi		; adding the second argument with the rax
			ret						; return from function, result stored in rax

subtract:
			mov		rax, rdi		; moving the first argument into the rax
			sub		rax, rsi		; adding the second argument with the rax
			ret						; return from function, result stored in rax
main:
			cmp	rdi, 4
...
add_handler: 
 			mov     rdi, r12    ; move the first number into rdi 
            mov     rsi, r14    ; move the second number into rsi
			call	add			; add(r12, r14)
            mov     r15, rax    ; store the result in r15
            jmp     format_result       ; jump to the format section for the result printing

sub_handler: 
			mov     rdi, r12    ; move the first number into rdi
            mov     rsi, r14    ; move the second number into rsi
			call	subtract	; subtract(r12, r14)
            mov     r15, rax    ; store the result in r15
            jmp     format_result       ; jump to the format section for the result printing
